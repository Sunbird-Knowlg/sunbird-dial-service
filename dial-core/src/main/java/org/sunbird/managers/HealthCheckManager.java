...
public class HealthCheckManager extends BaseManager implements IHealthCheckManager {
    ...
    public Response getAllServiceHealth(){
        ...
        try{
            ...
            check = generateCheck("org/sunbird/elasticsearch",check,status);
            TelemetryManager.log("org/sunbird/elasticsearch " +CONNECTION_SUCCESS);
        }catch (Exception e){
            check = generateCheck("org/sunbird/elasticsearch",check,status);
            TelemetryManager.error("org/sunbird/elasticsearch " +CONNECTION_FAIL,e);
        }
